...
import { createHeadlessRuntime } from '../../src/runtime/create_runtime.ts';
import { createGameStateStore } from '../../src/state/game_state_store.ts';
import type { HeadlessRuntime } from '../../src/contracts/ports.ts';
...
const playPath = (runtime: HeadlessRuntime, cells: Array<[number, number]>) => {
...
test('headless runtime scores unique infinite solutions and ignores equivalent duplicates', () => {
  ...
  const firstScoreState = (persistence.readBootState().scoreState as any);
  ...
  const duplicateScoreState = (persistence.readBootState().scoreState as any);
  ...
  const scoreState = (persistence.readBootState().scoreState as any);
