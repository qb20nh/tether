 ...
 */
const applyHistory = (
  controller: ReturnType<typeof createNotificationHistoryController>,
...
test('history controller renders localized entry text for mapped notification kinds', () => {
  ...
  const { controller, listEl } = createHarness({
    translateNow: (key: keyof typeof translations | string) => translations[key as keyof typeof translations] || key,
  });
  controller.bind();
...
test('history controller read-ack waits until unread rows are visibly rendered', async () => {
  ...
  windowObj.getComputedStyle = (node: FakeElement) => {
