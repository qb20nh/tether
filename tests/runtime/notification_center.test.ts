...
import {
  ...
  createWindowMock,
  flushMicrotasks,
} from './notification_test_harness.ts';

import type { NotificationHistoryEntry } from '../../src/contracts/ports.ts';
import type { NotificationHistoryPayload } from '../../src/contracts/ports.ts';
 ...
 */
const applyHistory = (
  center: ReturnType<typeof createNotificationCenter>,
  historyVersion: number,
  entries: NotificationHistoryEntry[],
) => {
  const payload: NotificationHistoryPayload = { historyVersion, entries };
  center.applyHistoryPayload(payload);
};

const createHarness = (overrides: any = {}) => {
  ...
  let applyCalls = ([] as any[]);
  let openCalls = ([] as any[]);
  ...
  const center = createNotificationCenter({
    ...
    translateNow: overrides.translateNow || ((key: string) => key),
    getLocale: () => 'en',
    showInAppToast: () => { },
    isOpenDailyHistoryActionable: overrides.isOpenDailyHistoryActionable || (() => true),
    onApplyUpdateRequested: overrides.onApplyUpdateRequested || (async (payload: unknown) => {
      applyCalls.push(payload);
    }),
    onOpenDailyRequested: overrides.onOpenDailyRequested || (async (payload: unknown) => {
      openCalls.push(payload);
    }),
    windowObj: (windowObj as any),
    documentObj: (documentObj as any),
  });
...
test('notification center normalizes and truncates history payloads to max entries', () => {
  ...
  const entries = ([] as NotificationHistoryEntry[]);
...
test('notification center dispatches history row actions to callbacks', async () => {
  const applyPayloads = ([] as any[]);
  const openPayloads = ([] as any[]);
  const { center, listEl } = createHarness({
    onApplyUpdateRequested: async (payload: unknown) => {
      applyPayloads.push(payload);
    },
    onOpenDailyRequested: async (payload: unknown) => {
...
test('notification center dialog helpers fall back to window.confirm when modal APIs are unavailable', async () => {
  let capturedApply = (null as any);
  let capturedOpenDaily = (null as any);
...
test('notification center refreshLocalizedUi re-renders history text using active locale translation', () => {
  ...
  const { center, listEl } = createHarness({
    translateNow: (key: string) => translations[locale][key] || key,
  });
