...
import type { NotificationPermissionState } from '../../src/contracts/ports.ts';
...
const createMessengerStub = (overrides: any = {}) => {
  let registration: unknown = overrides.registration || null;
  ...
  return {
    ...
    messenger: {
      ...
      setRegistration(nextRegistration: unknown) {
        registration = nextRegistration || null;
      },
      postMessage: async (message: { type: string; [key: string]: unknown }, options: Record<string, unknown> = {}) => {
        postedMessages.push({ message, options });
        return true;
      },
      postMessageWithReply: (...args: unknown[]) => postReply(...args),
...
const createOrchestrator = ({
  ...
  notificationPermission = ('default' as NotificationPermissionState),
  now = () => Date.now(),
  updateApplyReloadFallbackMs = 25,
}: any = {}) => createSwUpdateOrchestrator({
...
test('checkForNewBuild performs APPLY path when auto-update is enabled', async () => {
  const waitingWorkerMessages: Array<{ type?: string; [key: string]: unknown }> = [];
  const waitingWorker = {
    state: 'installed',
    addEventListener() { },
    removeEventListener() { },
...
test('ensureServiceWorkerUpdatePolicyConsistency resyncs only when policy drift is detected', async () => {
  ...
  const orchestrator = createOrchestrator({
    messenger: messengerHarness.messenger,
    readAutoUpdateEnabledPreference: () => false,
    shouldResyncManualUpdatePolicy: ({ swPolicy }: { swPolicy?: { autoUpdateEnabled?: boolean } | null }) => swPolicy?.autoUpdateEnabled === true,
  });
...
test('ensureServiceWorkerUpdatePolicyConsistency resyncs when pinned cache is unavailable', async () => {
  ...
  const orchestrator = createOrchestrator({
    messenger: messengerHarness.messenger,
    readAutoUpdateEnabledPreference: () => false,
    shouldResyncManualUpdatePolicy: ({ swPolicy }: { swPolicy?: { pinnedCacheUsable?: boolean } | null }) => swPolicy?.pinnedCacheUsable === false,
  });
...
test('applyUpdateForBuild triggers one-time reload fallback when no waiting worker is found', async () => {
  ...
  const orchestrator = createOrchestrator({
    messenger: messengerHarness.messenger,
    setUpdateProgressOverlayActive: (value: boolean) => overlayCalls.push(value),
    windowObj,
  });
