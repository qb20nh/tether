...
const repoFile = (...parts) => path.join(process.cwd(), ...parts);

const readJson = (...parts) => JSON.parse(fs.readFileSync(repoFile(...parts), 'utf8'));
const readText = (...parts) => fs.readFileSync(repoFile(...parts), 'utf8');
...
test('package metadata standardizes on pnpm and only-allow', () => {
  ...
  assert.equal(pkg.scripts['verify:release-no-debug'], 'node --import tsx scripts/verify_release_no_debug_artifacts.js');
