...
const createComputedStyle = (element: FakeElement | null | undefined) => ({
  getPropertyValue(name: string) {
...
const installRendererEnv = (t: { after: (cleanup: () => void) => void }) => {
  ...
  globalObject.getComputedStyle = (element: FakeElement) => (createComputedStyle(element) as any);
...
const flushNextTimeout = (env: { timeoutCallbacks: Map<number, () => void> }) => {
...
const createFakeWebgl2 = () => {
  ...
  return {
    ...
    gl: {
      ...
      shaderSource() { },
      compileShader() { },
      getShaderParameter(_shader: unknown, _pname: unknown) { return true; },
      getShaderInfoLog() { return ''; },
      deleteShader() { },
      createProgram() { return { id: nextId++ }; },
      attachShader() { },
      linkProgram() { },
      getProgramParameter(_program: unknown, _pname: unknown) { return true; },
...
const createFakePathRenderer = () => ({
  ...
  destroyCalls: ([] as any[]),
  resizeCalls: ([] as any[]),
  drawPathFrame(payload: Record<string, any>) {
    this.calls.push({
...
const createShellRefsWithWebglCanvas = () => {
  ...
  const contextOptions: Array<{ canvas: FakeElement; options: { antialias?: boolean } | undefined; stats: { drawCalls: number } }> = [];
  const installWebglContext = (canvas: FakeElement) => {
    ...
    canvas.getContext = (kind: string, options?: unknown) => {
      if (kind === 'webgl2') {
        ...
        const webglOptions = (typeof options === 'object' && options !== null)
          ? (options as { antialias?: boolean })
          : undefined;
        contextOptions.push({ canvas, options: webglOptions, stats: context.stats });
        return context.gl;
      }
  ...
  const documentRef = (globalObject.document as any);
  const originalCreateElement = documentRef.createElement;
  documentRef.createElement = (tagName: string) => {
...
const createSnapshot = (options: {
  ...
  const visited = new Set(path.map((point: { r: number; c: number }) => `${point.r},${point.c}`));
  const idxByKey = new Map(path.map((point: { r: number; c: number }, index: number) => [`${point.r},${point.c}`, index]));
  const totalUsable = gridData.flat().filter((cell: string) => cell !== '#' && cell !== 'm').length;
  return ({
    version: 1,
    levelIndex,
    rows,
    cols,
    pathKey: path.map((point: { r: number; c: number }) => `${point.r},${point.c};`).join(''),
    gridData,
    path,
    visited,
    idxByKey,
    stitches,
    cornerCounts,
    stitchSet: new Set(stitches.map((stitch: [number, number]) => `${stitch[0]},${stitch[1]}`)),
    stitchReq: new Map(),
    totalUsable,
  } as any);
};

const recordStrokeSegments = (ctx: any) => {
  ...
  let currentSegment = ({
    from: null,
    to: null,
  } as { from: { x: number; y: number } | null, to: { x: number; y: number } | null });
...
const getGridCell = (refs: { gridEl: FakeElement }, r: number, c: number, cols: number) => refs.gridEl.children[(r * cols) + c] || null;
const getBoardNavMarker = (refs: { boardHost: FakeElement }) => refs.boardHost.querySelector('.boardNavMarker');
const countBoardNavMarkers = (refs: { boardHost: FakeElement }) => (
  refs.boardHost.children.filter((child: FakeElement) => child.classList.contains('boardNavMarker')).length
);
const updateInteraction = (renderer: { updateInteraction: (payload: unknown) => void }, payload: unknown) => renderer.updateInteraction((payload as any));
const renderFrame = (renderer: { renderFrame: (payload: unknown) => void }, payload: unknown) => renderer.renderFrame((payload as any));
...
test('createBoardRendererCore keeps refs, interaction state, and ghosts isolated per instance', (t) => {
  ...
  const first = (createBoardRendererCore() as any);
  const second = (createBoardRendererCore() as any);
...
test('createBoardRendererCore positions wall ghosts from interaction coordinates', (t) => {
  ...
  const core = (createBoardRendererCore() as any);
  const refs = createShellRefs();
...
test('createBoardRendererCore applies incremental path patches for end/start batches and mixed endpoint turns', (t) => {
  ...
  const gridData = [['.', '.', '.', '.'], ['.', '.', '.', '.']];
  const core = (createBoardRendererCore({ debugCounters: counters }) as any);
  const refs = createShellRefs();
...
test('createBoardRendererCore does not rewrite message DOM when message content is unchanged', (t) => {
  ...
  refs.msgEl.classList.add = (...values: string[]) => {
    messageClassAdds += 1;
    return originalAdd(...values);
  };
  refs.msgEl.classList.remove = (...values: string[]) => {
...
test('createDomRenderer defers solved completion classes while a keyboard path tip is still selected', (t) => {
  ...
  const renderer = (createDomRenderer() as any);
  const refs = createShellRefs();
