...
const createRenderer = (
  canvas: { getContext: (kind: string, options?: { antialias?: boolean }) => unknown },
  options?: { antialias?: boolean },
) => (
  createPathWebglRenderer((canvas as any), options) as any
);
const drawPathFrame = (renderer: { drawPathFrame: (frame: unknown) => void }, frame: unknown) => renderer.drawPathFrame((frame as any));

type MeshWithIndices = { indices: ArrayLike<number>; vertexCount: number };
type MeshWithTravels = { travels: ArrayLike<number> };
type MeshWithPositionsAndTravels = {
  vertexCount: number;
  positions: ArrayLike<number>;
  travels: ArrayLike<number>;
};

const assertIndexBounds = (mesh: MeshWithIndices) => {
...
const assertTravelFinite = (mesh: MeshWithTravels) => {
  let minTravel = Infinity;
  let maxTravel = -Infinity;
  for (let i = 0; i < mesh.travels.length; i += 1) {
...
const createFakeWebgl2 = () => {
  ...
  const gl = {
    ...
    shaderSource() { },
    compileShader() { },
    getShaderParameter(_shader: unknown, pname: number) { return pname === gl.COMPILE_STATUS; },
    getShaderInfoLog() { return ''; },
    deleteShader() { },
    createProgram() { return { id: nextId++ }; },
    attachShader() { },
    linkProgram() { },
    getProgramParameter(_program: unknown, pname: number) { return pname === gl.LINK_STATUS; },
...
test('buildUnifiedPathMesh honors end arrow direction override', () => {
  ...
  const getApexDir = (mesh: MeshWithPositionsAndTravels) => {
    let apexIndex = 0;
...
test('buildUnifiedPathMesh honors start flow direction override', () => {
  ...
  const resolveStartCapTravel = (
    mesh: MeshWithPositionsAndTravels,
    scoreFn: (dx: number, dy: number) => number,
  ) => {
...
test('createPathWebglRenderer forwards the antialias option to WebGL2 context creation', () => {
  ...
  try {
    ...
    const contextOptions: Array<{ antialias?: boolean } | undefined> = [];
    const fakeCanvas = {
      width: 0,
      height: 0,
      clientWidth: 100,
      clientHeight: 100,
      style: {},
      getContext(kind: string, options?: { antialias?: boolean }) {
        if (kind === 'webgl2') {
