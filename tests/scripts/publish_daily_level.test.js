...
const writeFixtureHistory = (historyFile, entries) => {
  writeJson(historyFile, {
    schemaVersion: 1,
    entries,
  });
...
test('publish_daily_level keeps publishing past maxSlots by evicting oldest history', () => {
  ...
  assert.equal(history.entries.some((entry) => entry.dailyId === '2026-01-01'), false);
  assert.equal(history.entries.some((entry) => entry.dailyId === '2026-01-02'), true);
  assert.equal(history.entries.some((entry) => entry.dailyId === '2026-01-03'), true);
});
