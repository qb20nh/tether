...
import type { LevelDefinition, RuntimeIntent } from '../../src/contracts/ports.ts';
...
interface GridHarnessOptions {
  metrics?: Partial<GridMetrics>;
  elementFromPoint?: (x: number, y: number) => FakeElement | null;
  windowState?: WindowStateLike | null;
  level?: LevelDefinition;
  readLayoutMetrics?: () => unknown;
  emitIntent?: (intent: RuntimeIntent, store: ReturnType<typeof createGameStateStore>) => void;
}

type GameCommandIntentRecord = RuntimeIntent & {
  payload: {
    commandType: string;
    side?: string;
    steps?: Array<{ r: number; c: number }>;
    [key: string]: unknown;
  };
};

const isGameCommandIntent = (intent: RuntimeIntent): intent is GameCommandIntentRecord => (
  intent.type === INTENT_TYPES.GAME_COMMAND
  && !!intent.payload
  && typeof intent.payload === 'object'
  && 'commandType' in intent.payload
);

const hasUiActionPayload = (intent: RuntimeIntent): intent is RuntimeIntent & {
  payload: { actionType: string };
} => (
  !!intent.payload
  && typeof intent.payload === 'object'
  && 'actionType' in intent.payload
);
...
const createRefs = (gridEl: FakeElement) => ({
...
const createPointerEvent = (pointerId: number, clientX: number, clientY: number) => ({
  pointerId,
  clientX,
  clientY,
  cancelable: true,
...
const getLastIntent = (
  harness: { emittedIntents: RuntimeIntent[] },
  matcher: (intent: RuntimeIntent) => boolean,
): RuntimeIntent | null => {
...
const getLastBoardNavIntent = (harness: { emittedIntents: RuntimeIntent[] }) => (getLastIntent(
  harness,
  (intent) => (
...
const getLastInteractionIntent = (
  harness: { emittedIntents: RuntimeIntent[] },
  updateType: string,
) => (getLastIntent(
  harness,
...
const getLastWallDragIntent = (harness: { emittedIntents: RuntimeIntent[] }) => getLastInteractionIntent(
  harness,
  INTERACTION_UPDATES.WALL_DRAG,
);

const getLastWallDropTargetIntent = (harness: { emittedIntents: RuntimeIntent[] }) => getLastInteractionIntent(
  harness,
  INTERACTION_UPDATES.WALL_DROP_TARGET,
);

const getLastGameCommandIntent = (harness: { emittedIntents: RuntimeIntent[] }) => (getLastIntent(
  harness,
  (intent) => intent?.type === INTENT_TYPES.GAME_COMMAND,
) as any);

const tapDirectionalKeys = (
  harness: { gridEl: FakeElement; flushAllRafs: (timestamp: number, iterations?: number) => void },
  keys: string[],
  timestamp = 16,
) => {
  keys.forEach((key) => {
...
const installDomGlobals = (
  ...
  metrics: GridMetrics,
  elementFromPoint: (x: number, y: number) => FakeElement | null,
  windowState: WindowStateLike | null = null,
) => {
  ...
  const documentTarget = (new FakeElement('document') as any);
  ...
  return {
    ...
    windowTarget,
    rafCallbacks,
    setGamepads(gamepads: unknown[]) {
      currentGamepads = Array.isArray(gamepads) ? gamepads : [];
    },
  };
};

const createGridHarness = (
  t: { after: (cleanup: () => void) => void },
  options: GridHarnessOptions = {},
) => {
  const metrics = {
    version: 1,
    rows: 2,
  ...
  const emittedIntents: RuntimeIntent[] = [];
  const refs = createRefs(gridEl);

  const adapter = (createDomInputAdapter() as any);
  adapter.bind({
    refs,
    ...
    emitIntent: (intent: RuntimeIntent) => {
      ...
      if (!isGameCommandIntent(intent)) return;
      store.dispatch({
...
test('dom input adapter batches multi-cell drag moves into one sequence command', (t) => {
  ...
  const gameIntents = harness.emittedIntents.filter(isGameCommandIntent);
  assert.equal(gameIntents.length, 2);
  assert.ok(gameIntents[0]);
  assert.ok(gameIntents[1]);
...
test('dom input adapter ignores pointerdown within settings ui when deciding outside close', (t) => {
  ...
  assert.equal(
    harness.emittedIntents.some((intent) => hasUiActionPayload(intent) && intent.payload.actionType === UI_ACTIONS.SETTINGS_CLOSE),
    false,
  );
});
