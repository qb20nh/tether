...
const installControllableClock = async (page: Page) => {
  await page.addInitScript(() => {
    ...
    runtimeGlobal.__setTestNow = (value: number) => {
      nowMs = Number(value) || nowMs;
      return nowMs;
    };

    runtimeGlobal.__getTestNow = () => nowMs;
    runtimeGlobal.Date = (MockDate as DateConstructor);
  });
};

const getCellLocator = (page: Page, r: number, c: number) => page.locator(`[data-r="${r}"][data-c="${c}"]`);

test('e2e: daily locked board keeps walls and path tips visually distinct', async (t) => {
  let chromium: BrowserType = null as unknown as BrowserType;
  try {
  ...
  try {
    ...
    t.skip(`playwright could not launch in this environment: ${(error as any)?.message || error}`);
    return;
  }
  ...
  try {
    ...
    await page.evaluate((lockAt) => {
      (window as any).__setTestNow(lockAt + 2000);
    }, (lockAtUtcMs as number));
    ...
    const state = await page.evaluate(() => {
      const boardWrap = document.getElementById('grid')?.parentElement || null;
      const readCellStyle = (r: number, c: number) => {
